...
// 1 hole between : 2x16 teeths 
// type1 = "spur";
// tooth1 = 16;

// 2 holes between : 1x32 teeths 1x16
// type1 = "spur";
// tooth1 = 32;
// holeArray=1; // 0 => Only in center, 1 => 5 holes, 2 => 9 holes...


// 3 holes between : 1x48 teeths 1x16
// type1 = "spur";
// tooth1 = 48;
// holeArray=2; // 0 => Only in center, 1 => 5 holes, 2 => 9 holes...

// 4 holes between : 1x64 teeths 1x16
// type1 = "spur";
// tooth1 = 64;
// holeArray=3; // 0 => Only in center, 1 => 5 holes, 2 => 9 holes..
...
module hole() {
    translate([size/2, size/2, 0]) cylinder (h = size/4, r1=6.8/2, r2=5.2/2, center=false, $fn=100 );
    translate([size/2, size/2, size/4])  cylinder (h = size/4, r1=5.2/2, r2=6.8/2, center=false, $fn=100 );
}
...
size =8;
//spur_gear (modul=1, tooth_number=20, width=4.8, bore=8, pressure_angle=25, helix_angle=0, optimized=false);    
...
difference() {
    if (type1 == "spur") 
        spur_gear (modul=1, tooth_number=tooth1, width=4, bore=2, pressure_angle=25, helix_angle=0, optimized=false);
    if (type1 == "bevel") 
        bevel_gear(modul=1, tooth_number=tooth1,  partial_cone_angle=45, tooth_width=6.2, bore=2, pressure_angle=25, helix_angle=0);
    union() {
        translate([-size/2, -size/2, 0]) hole(); // center
        if (holeArray >= 1) {
            translate([size/2, -size/2, 0]) hole();
            translate([-3*size/2, -size/2, 0]) hole();        
            translate([-size/2, size/2, 0]) hole();                
            translate([-size/2, -3*size/2, 0]) hole();                                    
        }
        if (holeArray >= 2) {
            translate([3 * size/2, -size/2, 0]) hole();
            translate([-5 * size/2, -size/2, 0]) hole();        
            translate([-size/2, 3 * size/2, 0]) hole();                
            translate([-size/2, -5 * size/2, 0]) hole();                            
        }
        if (holeArray >= 3) {
            translate([5 * size/2, -size/2, 0]) hole();
            translate([-7 * size/2, -size/2, 0]) hole();        
            translate([-size/2, 5 * size/2, 0]) hole();                
            translate([-size/2, -7 * size/2, 0]) hole();                            
        }
    }
}
...
if (tooth2 > 0) {
    translate([0,0,size/2]) {
        difference() {
            if (type2 == "spur") 
                spur_gear (modul=1, tooth_number=tooth2, width=4, bore=2, pressure_angle=25, helix_angle=0, optimized=false);
            if (type2 == "bevel") 
                bevel_gear(modul=1, tooth_number=tooth2,  partial_cone_angle=45, tooth_width=6.2, bore=2, pressure_angle=25, helix_angle=0);
            union() {
                translate([-size/2, -size/2, 0]) hole(); // center
                if (holeArray >= 1) {
                    translate([size/2, -size/2, 0]) hole();
...
//bevel_gear(modul=2, tooth_number=14,  partial_cone_angle=45, tooth_width=5, bore=2, pressure_angle=25, helix_angle=0);
